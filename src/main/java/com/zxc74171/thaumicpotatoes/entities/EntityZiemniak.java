...
public class EntityZiemniak extends EntityMob implements IRangedAttackMob
{
    private static final DataParameter<Integer> FIRST_HEAD_TARGET = EntityDataManager.<Integer>createKey(EntityZiemniak.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> SECOND_HEAD_TARGET = EntityDataManager.<Integer>createKey(EntityZiemniak.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> THIRD_HEAD_TARGET = EntityDataManager.<Integer>createKey(EntityZiemniak.class, DataSerializers.VARINT);
    ...
    private static final DataParameter<Integer> INVULNERABILITY_TIME = EntityDataManager.<Integer>createKey(EntityZiemniak.class, DataSerializers.VARINT);
    ...
    public EntityZiemniak(World worldIn)
    {
        super(worldIn);
    ...
    {
        this.tasks.addTask(0, new EntityZiemniak.AIDoNothing());
    ...
    {
        EntityLiving.registerFixesMob(fixer, EntityZiemniak.class);
    }
    ...
    {
        return ModSoundEvents.ENTITY_ZIEMNIAK_AMBIENT;
    }

    protected SoundEvent getHurtSound(DamageSource p_184601_1_)
    {
        return ModSoundEvents.ENTITY_ZIEMNIAK_HURT;
    }

    protected SoundEvent getDeathSound()
    {
        return ModSoundEvents.ENTITY_ZIEMNIAK_DEATH;
    }
    ...
    public void onLivingUpdate()
    {
        ...
        for (int l = 0; l < 3; ++l)
        {
            ...
            this.world.spawnParticle(EnumParticleTypes.ENCHANTMENT_TABLE, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.0D, 0.0D, 0.0D);
                ...
                if (this.ticksExisted >= this.nextHeadUpdate[i - 1])
                {
                    ...
                    if (this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD)
                    {
                        ...
                        if (k3 > 15)
                        {
                            ...
                            this.launchPotatoToCoords(i + 1, d0, d1, d2, true);
                    ...
                    if (k1 > 0)
                    {
                        ...
                        if (entity != null && entity.isEntityAlive() && this.getDistanceSqToEntity(entity) <= 900.0D && this.canEntityBeSeen(entity))
                        {
                            ...
                            {
                                this.launchPotatoToEntity(i + 1, (EntityLivingBase)entity);
                    ...
                    {
                        List<EntityLivingBase> list = this.world.<EntityLivingBase>getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().grow(20.0D, 8.0D, 20.0D), Predicates.and(EntitySelectors.NOT_SPECTATING));
    ...
    private void launchPotatoToEntity(int p_82216_1_, EntityLivingBase p_82216_2_)
    {
        this.launchPotatoToCoords(p_82216_1_, p_82216_2_.posX, p_82216_2_.posY + (double)p_82216_2_.getEyeHeight() * 0.5D, p_82216_2_.posZ, p_82216_1_ == 0 && this.rand.nextFloat() < 0.001F);
    }
    
    /**
     * Launches a Wither skull toward (par2, par4, par6)
     */
    private void launchPotatoToCoords(int p_82209_1_, double x, double y, double z, boolean invulnerable)
    {
        ...
        double d4 = y - d1;
        double d5 = z - d2;
        EntityPotatoMobProjectile entitypotatomobprojectile = new EntityPotatoMobProjectile(this.world, this, d3, d4, d5);

        if (invulnerable)
        {
            entitypotatomobprojectile.setInvulnerable(true);
        }

        entitypotatomobprojectile.posY = d1;
        entitypotatomobprojectile.posX = d0;
        entitypotatomobprojectile.posZ = d2;
        this.world.spawnEntity(entitypotatomobprojectile);
    }
    ...
    {
        this.launchPotatoToEntity(0, target);
    }
    ...
    class AIDoNothing extends EntityAIBase
    {
        ...
        {
            return EntityZiemniak.this.getInvulTime() > 0;
        }
    }
}