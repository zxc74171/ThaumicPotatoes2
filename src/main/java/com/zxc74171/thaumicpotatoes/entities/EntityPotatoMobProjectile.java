...
public class EntityPotatoMobProjectile extends EntityFireball
{
    private static final DataParameter<Boolean> INVULNERABLE = EntityDataManager.<Boolean>createKey(EntityPotatoMobProjectile.class, DataSerializers.BOOLEAN);

    public EntityPotatoMobProjectile(World worldIn)
    {
        super(worldIn);
        this.setSize(0.3125F, 0.3125F);
    }

    public EntityPotatoMobProjectile(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ)
    {
    ...
    public EntityPotatoMobProjectile(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ)
    {
        ...
        if (!this.world.isRemote)
        {
            if (result.entityHit != null)
            {
                ...
                if (result.entityHit instanceof EntityLivingBase)
                {
                    ...
                    if (i > 0)
                    {
                        ((EntityLivingBase)result.entityHit).addPotionEffect(new PotionEffect(MobEffects.HUNGER, 20 * i, 1));
                    }
                }
            }
